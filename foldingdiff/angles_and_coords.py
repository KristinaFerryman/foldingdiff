...
def decode_sequence(sequence):
    sequence_new = [str(np.array(list(seq2angle.keys()))[np.argmin(np.abs(np.array(list(seq2angle.values()))-x))]) for x in sequence]
    return(np.array(sequence_new))
...
def create_new_chain_nerf_sequence(
    out_fname: str,
    out_fname_angles:str,
    out_fname_fasta:str,
    dists_and_angles: pd.DataFrame,
    angles_to_set: Optional[List[str]] = None,
    dists_to_set: Optional[List[str]] = None,
    ...
    the path to the newly create file if successful, empty string if fails.
    """
    sequence_angles = dists_and_angles.iloc[:, -1].values
    sequence1l = decode_sequence(sequence_angles)
    sequence = np.array([ProteinSequence.convert_letter_1to3(x) for x in sequence1l])
    dists_and_angles = dists_and_angles.iloc[:, :-1]
    if angles_to_set is None and dists_to_set is None:
        angles_to_set, dists_to_set = [], []
        for c in dists_and_angles.columns:
            # Distances are always specified using one : separating two atoms
    ...
    return write_coords_to_pdb_sequence(coords, sequence,
                                        sequence_angles,
                                        sequence1l, out_fname,
                                        out_fname_angles, out_fname_fasta)
...
def write_coords_to_pdb_sequence(coords: np.ndarray,
                                sequence:np.ndarray,
                                sequence_angles:np.ndarray,
                                sequence1l:np.ndarray,
                                out_fname: str,
                                out_fname_angles: str,
                                out_fname_fasta: str) -> str:
    """
    Write the coordinates to the given pdb fname
    """
    # Create a new PDB file using biotite
    # https://www.biotite-python.org/tutorial/target/index.html#creating-structures
    assert len(coords) % 3 == 0, f"Expected 3N coords, got {len(coords)}"
    assert len(coords)==(len(sequence)*3), f"Coords length mismatch sequence length"
    atoms = []
    for i, (n_coord, ca_coord, c_coord) in enumerate(
        (coords[j : j + 3] for j in range(0, len(coords), 3))
    ):
    ...
    np.savez_compressed(out_fname_angles, arr=sequence_angles)

    with open(out_fname_fasta, "w") as file:
        file.write(">seq\n")
        file.write("".join(list(sequence1l)))
